...
from censyspy.formatter import format_results
from censyspy.models import CertificateMatch, DNSMatch
...
def process_domain_results(
    ...
    """
    logger.info(
        f"Processing results for {domain}: {len(dns_results)} DNS records, "
    ...
    if expand_wildcards:
        ...
        if wildcard_diff != 0:
            logger.info(
                f"Wildcard processing: {wildcard_diff} wildcards expanded, "
...
def _process_dns_records(
    dns_results: List[Dict[str, Any]], domain: str
) -> Dict[str, DNSMatch]:
    """
...
def _process_certificate_records(
    cert_results: List[Dict[str, Any]], domain: str
...
def process_and_format(
    ...
    **format_options: Any,
    ...
    logger.debug(
        f"Formatting {len(dns_matches)} DNS matches and {len(cert_matches)} certificate matches"
    )
